...
/**
 * A lens with proper optical properties for ray tracing.
 */
export class Lens extends SceneEntity {
  ...
  // Getters for ray tracing access
  get diameter(): number {
    return this.opts.diameter;
  }
  get thickness(): number {
    return this.opts.thickness;
  }
  get frontSurface(): SurfaceSpec {
    return this.opts.frontSurface;
  }
  get backSurface(): SurfaceSpec {
    return this.opts.backSurface;
  }
  get glass(): GlassSpec {
    return this.opts.glass;
  }
  get position(): Vector3 {
    return this.opts.position || Vector3.Zero();
  }
  get rotation(): Vector3 {
    return this.opts.rotation || Vector3.Zero();
  }
  ...
  /**
   * Calculate refractive index at a given wavelength using Sellmeier equation
   */
  getRefractiveIndex(wavelength: number): number {
    ...
    const { B1, B2, B3, C1, C2, C3 } = this.opts.glass.dispersion;

    // Sellmeier equation: n² = 1 + B1*λ²/(λ²-C1) + B2*λ²/(λ²-C2) + B3*λ²/(λ²-C3)
    const n2 =
      1 +
  ...
  addToScene(scene: Scene): void {
    ...
    // Store optical metadata for ray tracing
    this.mesh.metadata = {
      type: "Lens",
      diameter,
      thickness,
      ...
      lensObject: this, // Reference to this lens instance
    };
  }
  ...
  private generateSurfaceIndices(
    ...
    for (let r = 0; r < radialSegments; r++) {
      for (let c = 0; c < circumferentialSegments; c++) {
        ...
        const next =
          vertexOffset +
          r * circumferentialSegments +
          ((c + 1) % circumferentialSegments);
        const currentNext =
          vertexOffset + (r + 1) * circumferentialSegments + c;
        const nextNext =
          vertexOffset +
          (r + 1) * circumferentialSegments +
          ((c + 1) % circumferentialSegments);
  ...
  private generateEdgeVertices(
    ...
    for (let c = 0; c < circumferentialSegments; c++) {
      ...
      const frontEdgeZ = this.calculateSurfaceZAtRadius(
        -this.opts.frontSurface.radius,
        frontZ,
        radius
      );
      const backEdgeZ = this.calculateSurfaceZAtRadius(
        -this.opts.backSurface.radius,
        backZ,
        radius
      );
  ...
  private generateSurfaceVertices(
    ...
    for (let r = 0; r <= radialSegments; r++) {
      ...
      for (let c = 0; c < circumferentialSegments; c++) {
        ...
        const z = this.calculateSurfaceZAtRadius(
          radiusOfCurvature,
          centerZ,
          currentRadius
        );
        ...
        if (radiusOfCurvature === 0) {
          ...
          const centerY = 0;
          const centerZ =
            z + (isFrontSurface ? -radiusOfCurvature : radiusOfCurvature);
   ...
   */
  calculateSurfaceZAtRadius(
    radiusOfCurvature: number,
    centerZ: number,
    radius: number
  ): number {
    if (radiusOfCurvature === 0) return centerZ; // flat
    ...
    if (radius >= R) {
      ...
      const discr = 1 - r2 / (R * R);
      sag = r2 / (2 * R * (1 + Math.sqrt(discr)));
    }
  ...
  private generateEdgeIndices(
    ...
    for (let c = 0; c < circumferentialSegments; c++) {
      ...
      const nextBack =
        vertexOffset + ((c + 1) % circumferentialSegments) * 2 + 1;
